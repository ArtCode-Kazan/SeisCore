...
output_file_path = r'/media/michael/Data/TEMP/energy.dat'
t_start = 10
freq_limits = [(0, 25), (2, 3), (3, 4), (18.3, 19.4)]
...
for index in range(len(sensor_numbers)):
    signal = signal_data[:, 1 + index]
    x, y = coords_data[sensor_numbers[index]]
    # sp = spectrum(signal=signal, frequency=frequency)
    sp = average_spectrum(signal=signal, frequency=frequency,
                          window=4096, offset=1024)
...
np.savetxt(output_file_path, result, delimiter='\t', header=header,
           comments='')
