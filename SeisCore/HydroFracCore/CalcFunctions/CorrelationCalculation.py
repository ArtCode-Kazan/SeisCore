...
def calc_correlation(point_numbers, base_point_number, signals, frequency,
    ...
    if base_point_number not in point_numbers:
        return None
    # Проверка, что число столбцов массива с сигналами равно числу точек в списке
    ...
    # Обход производится по каждому отсчету базового датчика, начиная от начала минуты
    # НЕ ОТ СНАЧАЛА СИГНАЛА, а от начала минуты. Так как извлеченный сигнал содержит буферы
    for base_moment in range(left_buffer, 60 * frequency + left_buffer):
        ...
        for point in point_numbers:
            ...
            for current_delay in range(min_moment_delay, max_moment_delay + 1, 1):
                ...
                signal_b = current_point_signal[left_edge + current_delay:right_edge + current_delay]
        ...
        # Добавление результата корреляции в файл
