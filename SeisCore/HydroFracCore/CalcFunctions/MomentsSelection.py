# coding=utf-8
"""
Данный модуль содержит функции для первичной обработки данных ГРП - отбор моментов событий на основе
максимальных значений квадратов коэф-тов кросскорреляции
"""
...
def quantilies(data, procents=[95, 96, 97, 98, 99]):
    ...
    :param data: одномерный массив numpy со значениями максимальных квадратов коэф-тов кросскорреляции
    :param procents: список с целыми числами - проценты, для которых нужно найти квантили.
...
def get_intervals(moments_numbers, epsilon=1):
    """
    Функция для представления номеров отсчетов сигналов (отсортированных) в виде интервалов
    :param moments_numbers: отсортированный одномерный массив numpy с номерами отсчетов сигнала
    :param epsilon: допуск для склеивания текущего номера с предыдущим интервалом (по умолчанию=1 дискрета)
    :return: двухмерный массив numpy с границами интервалов (границы включительны!!!)
    """
...
def quantile_intervals(data, quantile, epsilon=0):
    """
    Функция для получения интервалов индексов моментов, где значение корреляции больше или равно значению квантиля
    :param data: одномерный массив numpy со значениями максимальных квадратов коэф-тов кросскорреляции
    :param quantile: значение квантиля (НЕ ПРОЦЕНТ!, а значение)
    :param epsilon: допуск отклонения корреляции от квантиля
    :return:
    """
    selection_indexes = np.where((abs(data - quantile) <= epsilon) + (data >= quantile))[0]
...
def is_crossing_intervals(interval_a, interval_b):
    ...
    :param interval_a: список с пределами интервала (интервал включительно с границами)
    :param interval_b: список с пределами интервала (интервал включительно с границами)
    :return: True - если находится, False - если нет
    """
    if (interval_b[0] <= interval_a[0]) and (interval_b[0] < interval_a[1]) and (interval_b[1] > interval_a[0]) and \
...
def intervals_intersection(intervals):
    ...
    for i in range(intervals.shape[0]):
        ...
        for j in range(intervals.shape[0]):
            ...
            # если интервал A входит в интервал B, то интервал B помечается на удаление,
            # так как он шире, чем интервал A
            if is_crossing_intervals(interval_a, interval_b) and (i != j):
                mask_to_delete[j] = True

            # если интервал B входит в интервал A, то интервал A помечается на удаление,
            # так как он шире, чем интервал B
...
def max_value_index(data, interval=None):
    ...
    :param data: одномерный массив numpy со значениями максимальных квадратов коэф-тов кросскорреляции
    :param interval: интервал массива  в виде списка, где необходимо выполнить поиск (границы включительно!)
...
def moments_selection(data, procents=[95, 96, 97, 98, 99], epsilon=0):
    ...
    :param epsilon: допуск отклонения коэ-тов корреляции от значения квантиля при выборке
    :return: одномерный numpy массив с номерами индексов отсчетов (нумерация идет от ноля)
    """
    # расчет квантилей
    ...
    for i in range(len(quants)):
        ins = quantile_intervals(data=data, quantile=quants[i], epsilon=epsilon)
    ...
    # поиск индекса максимального элемента массива в выбранных интервалах пересечений
    result = np.empty(shape=0, dtype=int)
    for interval in intersections:
        left_edge = interval[0]
        right_edge = interval[1] + 1  # +1, чтобы получить полуоткрытый интервал
        index_with_max_value = max_value_index(data, interval=[left_edge, right_edge])
